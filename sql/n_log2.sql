...
--
-- Host: 127.0.0.1
-- Generation Time: 2016-01-08 04:17:50
-- 服务器版本： 5.6.20
-- PHP Version: 5.5.15
...
--
-- Database: `gbosng2_log`
--
CREATE DATABASE IF NOT EXISTS `gbosng2_log` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `gbosng2_log`;
...
-- Database: `gbosng2`
--
CREATE DATABASE IF NOT EXISTS `gbosng2` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `gbosng2`;
...
DROP PROCEDURE IF EXISTS `pro_rnkScore`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `pro_rnkScore`()
BEGIN
	DECLARE rnk varchar(32) character set utf8;
    SET rnk = concat("gbosng2_log.rankscore",DATE_FORMAT(NOW(),'%Y%m%d'));
...
DROP PROCEDURE IF EXISTS `pro_rnkSword`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `pro_rnkSword`()
BEGIN
	DECLARE rnk varchar(32) character set utf8;
    SET rnk = concat("gbosng2_log.ranksword",DATE_FORMAT(NOW(),'%Y%m%d'));
...
BEGIN
	DECLARE rnk varchar(64) character set utf8;
    SET rnk = concat("gbosng2_log.rankwheel",DATE_FORMAT(NOW(),'%Y%m%d'));
